...
def init_asset(*, output: abi.Uint64) -> Expr:
    ...
    return Seq(
        ...
        # App.globalPut(Bytes("exchange_rate"), Mul(Int(INITIAL_EXCHANGE_RATE), SCALING_FACTOR)),
...
def algos_to_asset(algo_amount: Expr) -> Expr:
    # return Div(Mul(algo_amount, App.globalGet(Bytes("exchange_rate"))), SCALING_FACTOR)
    return Div(algo_amount, App.globalGet(Bytes("exchange_rate")))


@Subroutine(TealType.uint64)
def asset_to_algos(asset_amount: Expr) -> Expr:
    # return Div(Mul(asset_amount, SCALING_FACTOR), App.globalGet(Bytes("exchange_rate")))
