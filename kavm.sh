...
run_kompile() {
    ...
    case "$backend" in
        llvm) kompile_opts+=( -ccopt -I${libff_dir}/include -ccopt -L${libff_dir}/lib              )
...
run_krun() {
    ...
    krun_args+=(-pTEAL_PROGRAMS="$INSTALL_LIB/scripts/parse-teal-programs.sh")
    # run_file will contain an AVM simulation scenario,
    # to be parsed with parse-avm-simulation.sh
    krun_args+=(--parser "$INSTALL_LIB/scripts/parse-avm-simulation.sh")
...
export backend_dir="${backend_dir:-${INSTALL_LIB}/avm-$backend}"
